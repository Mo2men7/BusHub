...
class TripAdminController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($allTrips as $key => $trip) {
            ...
            $details = DB::table('trips')->join('types', 'buses.type_id', '=', 'types.id')->select(['trips.*', 'buses.id as bus_id', 'buses.chairs', 'buses.type_id', 'types.type', 'types.options'])
                ->where('to', '=', $trip->to)
    ...
    {
        $check = DB::table('trips')->select()->where('bus_id', $request->bus_id)->where('date', $request->date)->where('time', $request->time)->where('to', $request->to)->where('from', $request->from)->get()->toArray();
        if (count($check) > 0) {
        ...
        for ($i = 1; $i <= $intChairs; $i++) {
            ...
            $queryToIns = Seat::create($insertRow);
        }
    ...
    public function update(Request $request, string $id)
    {
        if (Trip::where('id', $id)->exists()) {
            $check = DB::table('trips')->select()->where('bus_id', $request->bus_id)->where('date', $request->date)->where('time', $request->time)->where('to', $request->to)->where('id', '<>', $id)->where('from', $request->from)->get()->toArray();
            if (count($check) > 0) {
                return response()->json(["error" => "this Trip is already exists"], 402);
            }
            $trip = Trip::find($id);
